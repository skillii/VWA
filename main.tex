...
\sep EOG
\sep BSS
\sep Differential Method
\sep Least Square Method
\sep Inverse Filtering
\sep Morphological Component Analysis
...
Bei dieser Methode wird das gemessene EEG-Signal ($y(n)$) als Superposition des EOG-Signals ($u(n)$) und dem echten EEG-Signals
 ...
 echten EEG-Signals. Es wird nun das EEG-Signal wie eine Störung behandelt, wie in Gleichung~\ref{eq:LS-Method} zu sehen ist. Man verwendet nun die Methode der kleinsten
 Quadrate um den Faktor $b_i$ zu berechnen! Mit den nun bekannten Faktoren kann man das echte EEG-Signal ($w(n)$) berechnen.
(In der Berechnung wird die Zeit diskretisiert und mit $n$ dargestellt.)~\cite{Thulasidas2004}
\begin{equation} \label{eq:LS-Method}
 y(n) =  \sum_{i=1}^{N} b_i * u_i(n) + w(n)
\end{equation}

Man muss dazu aber folgende (vereinfachte) Annahmen treffen \cite{Thulasidas2004}:
\begin{itemize}
 \item Das echte EEG ist frei von Gleichanteilen
 \item Faktor $b_i$ ist Frequenz- und Zeitunabhängig.
...
Die Differenz Methode leitet sich von der LS-Methode ab. Durch diese Methode beseitigt man die zeitliche Abhängigkeit der aufeinanderfolgenden Samples.
~\cite{Thulasidas2004}
...
Die gemessenen EEG-Daten ($Y$-Matrix) werden in ihre Komponenten zerlegt in dem man sie mit einer speziellen (Dreh)-Matrix $W$ multipliziert (siehe Gleichung \ref{eq:BSS}).
...
Berechnungszeit und der Algorithmuskomplexität.
Desshalb ist diese Methode eher weniger für Online-Implementierungen geeignet.

Ein direkter Leistungsvergleich der genannten Methode ist schwierig durchführbar, da sich die Methoden für 
Artefakte unterschiedlichen Ursprungs, unterschiedlich gut eignen. Eine Kombination mehrerer Methoden ist denkbar.
